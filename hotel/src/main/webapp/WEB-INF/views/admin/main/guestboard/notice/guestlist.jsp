										...
										<c:forEach items="${data.list }" var="vo" varStatus="status">
												...
												<tr style="background-color: pink" style="font-weight":bold" >
													...
													<td class="txt_l"><a
														href="/hotel/guestnotice/view.do?gnotice_no=${vo.gnotice_no}">
															<img src="/hotel/image/boardPic/느낌2.png" width="20px">
															<b>${vo.gnotice_title}</b> <img
															src="/hotel/image/boardPic/new2.png" width="30px">
													</a></td>
											...
											<c:if test="${vo.fix != 1}">
												<tr>
													<td>${data.totalCount - status.index - ((guestNoticeVO.page - 1) * guestNoticeVO.pageRow)}<!-- 계산식 = "총개수 - 인덱스 - (현재 페이지 번호 - 1) * 페이지당 개수" --></td>
													<td class="txt_l"><a
														href="/hotel/guestnotice/view.do?gnotice_no=${vo.gnotice_no}">${vo.gnotice_title}
															<c:if test="${vo.diff <= 3 }">
																<img src="/hotel/image/boardPic/new (4).png"
																	width="30px">
															</c:if>
													</a></td>
													<td class="writer">관리자</td>
